...
let loopButton = document.getElementById("loopButton"); /*retrieve reference for the loop button*/
...
/* MUTE/UNMUTE */
/*Affordance - I want to toggle mute and unmute
Signifier - I want to press a button
Feedback - I want to change out the icon*/
function muteUnmute() {
...
/*add event listener: click button to activate function*/
  muteButton.addEventListener('click', muteUnmute);

/* Add event listener for Controls Wrapper*/
//when mouse hovers over Control Wrapper element, activate function mouseOver
controlsWrapper.addEventListener("mouseover", mouseOver); 
//when mouse moves out of Control Wrapper element, activate function mouseOut
controlsWrapper.addEventListener("mouseout", mouseOut);

/* Adjust Control Wrapper's Visibility */
/* I think the below function benefits visual hierarchy in guiding users to focus more on the video element than the control wrapper */
/* I want the Control Wrapper's opacity to be 1 when the page is loaded so that the user sees the full buttons and progress bar, but when 
the mouse enters the wrapper box, I want its opacity to decrease so that the user focuses more on the video itself */
function mouseOver(){
  /* The control wrapper decreases in opacity but still needs to be visible enough
  so that users can see the buttons and their feedbacks.  */
 controlsWrapper.style.opacity = 0.7;
}

function mouseOut(){
  /* Setting opacity to "0" so that it won't cover the content of the video when the user is done disabling and enabling the buttons */
  controlsWrapper.style.opacity = 0;                                  
} 

/* LOOP/UNLOOP */
/* I select this extra feature because sometimes when I go on YouTube, I find that there are music videos edited to loop for 1 hour
made by other users, so clearly this is a need for some users. However, this method is limited and some people don't even know their 
free account has loop available because there isn't any signifier to let them know. 
Although on YouTube users can actually loop the video by right-click on the video itselfand click on "Loop", I think this still takes so many steps, 
as there is no button to toggle between enable and disable loop.
Therefore, I think this feature will offer a shortcut to help looping more accessible and enjoyable for users.
- Affordance - I want to toggle between loop and unloop
- Signifier - I want to click on the button 
- Feedback - I want to change the icons */ 
function loopUnLoop() {
  // the following if statement checks to see if the media is currently looping
  if(videoElement.loop) {
    /* if the video is looping, set to disable loop */
    videoElement.loop = false;
    loopButton.style.backgroundImage = "url('./icons/unloop.svg')";
    /* Observing YouTube's media player, I find that it assigns short description to each icon, which is helpful in letting users know what stage 
    the button will change to*/
    loopButton.title = "Loop";
  }
  else {
    /* if the video is not looping, set to enable loop */
    videoElement.loop = true;
    loopButton.style.backgroundImage = "url('./icons/loop.svg')";
    loopButton.title = "Unloop";
  }
}
/* Add event listener: click button to activate function  */
